...
void TestCos::initTestCase()
{
    ...
    FileHelper::writeFlie(QStringList() << "abcdef", m_uploadLocalPath);
}
...
//    QCOMPARE(bs.size(), 2);  // QCOMPARE 进行恒等的比较
}
...
void TestCos::test_getObjects()
{
    ...
    objList = m_cos.getObjects(m_bucketName, "test1/");
    QCOMPARE(objList.size(), 2);
